$(document).ready(function () {
    ...
    async function loadJadwalPerHari() {
        ...
        try {
            const res = await fetch(`/seminar-proposal?tahun_akademik=${tahun_akademik}`, {
                headers: {
    ...
    $('#jadwalContainer').on('click', '.btnDetail', async function () {
        ...
        try {
            const res = await fetch(`/seminar-proposal/${id}`, {
                method: 'POST',
                headers: {
            ...
            if (!data.proposal || data.proposal.length === 0) {
                ...
                const checkbox = data.proposal.status_proposal === 'MENUNGGU' ?
                    `<div class="col"><button type="button" class="btn btn-success btn-sm btn-flat cekPersetujuan" data-proposal="${data.proposal.id}" data-action="approve">Setujui</button>
                    <button type="button" class="btn btn-danger btn-sm btn-flat cekPersetujuan" data-proposal="${data.proposal.id}" data-action="reject">Tolak</button></div>` :
                    data.proposal.status_proposal === 'DISETUJUI' ?
                        `<div class="col"><button type="button" class="btn btn-danger btn-sm btn-flat cekPersetujuan" data-proposal="${data.proposal.id}" data-action="batal">Batalkan</button></div>` :
    ...
    $('#modalKRSBody').on('click', '.cekPersetujuan', async function () {
        ...
        try {
            const res = await fetch('/seminar-proposal/proposal/update-status', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-Requested-With': 'XMLHttpRequest',
