...
@section('content')
    <div class="card">
        <div class="card-header d-flex align-items-center">
            <h6 class="mb-0">Jadwal Pembayaran</h6>
            <div class="ms-auto">
                @can($modul . '.create')
                    <a href="{{ route($modul . '.create') }}" class="btn btn-sm btn-primary">Tambah Jadwal Pembayaran</a>
                @endcan
            </div>
        </div>
        <div class="card-body">
                    ...
                    <tbody>
                        @foreach ($jadwal_pembayaran as $item)
                            <tr>
                                ...
                                <td>
                                    @foreach ($item->program_studi_detail as $val)
                                        <small class="d-block">
                                            {{ $val->nama_program_studi_idn }}
                                        </small>
                                    @endforeach
                                </td>
                                @canany([$modul . '.destroy', $modul . '.edit'])
